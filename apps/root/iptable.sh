...
if [[ $iptable_mangle_exit_code != 0 ]]; then
	...
	echo "[warn] iptable_mangle module not supported, you will not be able to connect to transmission or Privoxy outside of your LAN"
...
# accept input to transmission port 9091
iptables -A INPUT -i eth0 -p tcp --dport 9091 -j ACCEPT
iptables -A INPUT -i eth0 -p tcp --sport 9091 -j ACCEPT
...
if [[ $iptable_mangle_exit_code == 0 ]]; then
	...
	# accept output from transmission port 9091 - used for external access
	iptables -t mangle -A OUTPUT -p tcp --dport 9091 -j MARK --set-mark 1
	iptables -t mangle -A OUTPUT -p tcp --sport 9091 -j MARK --set-mark 1

fi

# accept output from transmission port 9080 - used for lan access
iptables -A OUTPUT -o eth0 -p tcp --dport 9091 -j ACCEPT
